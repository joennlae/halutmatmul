...
BoltEncoder::BoltEncoder(int nbytes, float scaleby)
    : _offsets(2 * nbytes), _lut(16, 2 * nbytes), _scaleby(scaleby),
      _nbytes(nbytes) {
  bool valid = (nbytes == 2 || nbytes == 8 || nbytes == 16 || nbytes == 24 ||
                nbytes == 32);
  if (!valid) {
    printf("ERROR: Received invalid nbytes %d; "
           "must be one of {2, 8, 16, 24, 32}.",
           nbytes);
    exit(1);
  }
  _offsets.setZero();
...
void lut(const float *q, int len, int nbytes, const RowMatrix<float> &centroids,
         const RowVector<float> &offsets, float scaleby,
         ColMatrix<uint8_t> &lut_out) {
  assert(scaleby > 0.);
...
void BoltEncoder::lut_l2(const float *q, int len) {
  lut<Reductions::DistL2>(q, len, _nbytes, _centroids, _offsets, _scaleby,
                          _lut);
}
void BoltEncoder::lut_dot(const float *q, int len) {
  lut<Reductions::DotProd>(q, len, _nbytes, _centroids, _offsets, _scaleby,
                           _lut);
}
...
void query(const float *q, int len, int nbytes,
           ...
           float scaleby, const RowMatrix<uint8_t> &codes, int64_t ncodes,
           ColMatrix<uint8_t> &lut_tmp, uint16_t *dists) {
  assert(nbytes > 0);
  assert(scaleby > 0);
  assert(codes.rows() > 0);
  assert(codes.cols() > 0);
...
RowVector<uint16_t> query_all(const float *q, int len, int nbytes,
  ...
  RowVector<uint16_t> dists(
      codes.rows()); // need 32B alignment, so can't use stl vector
  dists.setZero();
...
template <int Reduction = Reductions::DistL2>
vector<int64_t>
query_knn(const float *q, int len, int nbytes,
          const RowMatrix<float> &centroids, const RowVector<float> &offsets,
          float scaleby, const RowMatrix<uint8_t> &codes, int64_t ncodes, int k,
          ColMatrix<uint8_t> &lut_tmp, bool smaller_better = true) {
  RowVector<uint16_t> dists(
      codes.rows()); // need 32B alignment, so can't use stl vector
  query<Reduction>(q, len, nbytes, centroids, offsets, scaleby, codes, ncodes,
                   lut_tmp, dists.data());
  ...
  vector<nn::Neighbor> neighbors =
      nn::knn_from_dists(dists.data(), ncodes, k, smaller_better);
...
RowVector<uint16_t> BoltEncoder::dists_sq(const float *q, int len) {
  return query_all<Reductions::DistL2>(q, len, _nbytes, _centroids, _offsets,
                                       _scaleby, _codes, _ncodes, _lut);
}
RowVector<uint16_t> BoltEncoder::dot_prods(const float *q, int len) {
  return query_all<Reductions::DotProd>(q, len, _nbytes, _centroids, _offsets,
                                        _scaleby, _codes, _ncodes, _lut);
}

vector<int64_t> BoltEncoder::knn_l2(const float *q, int len, int k) {
  return query_knn<Reductions::DistL2>(q, len, _nbytes, _centroids, _offsets,
                                       _scaleby, _codes, _ncodes, k, _lut);
}
vector<int64_t> BoltEncoder::knn_mips(const float *q, int len, int k) {
  bool smaller_better = false;
  return query_knn<Reductions::DotProd>(q, len, _nbytes, _centroids, _offsets,
                                        _scaleby, _codes, _ncodes, k, _lut,
                                        smaller_better);
}
