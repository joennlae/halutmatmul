...
int main(int argc, char *const argv[]) {
  ...
  REPEATED_PROFILE_DIST_COMPUTATION(
      kNreps, "bolt scan uint8                  ", kNtrials, dists_u8.data(),
      nrows,
      (bolt_scan<M, true, signed_luts>(codes.data(), luts.data(),
                                       dists_u8.data(), nblocks)));

  // RowVector<uint8_t> dists_u8_x2(nrows * 2); // incase decides to upcast
  // REPEATED_PROFILE_DIST_COMPUTATION(kNreps, "bolt scan avg upcast=4 ",
  // kNtrials,
