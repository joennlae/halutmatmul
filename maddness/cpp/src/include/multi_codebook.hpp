...
template <int NBytes>
inline void
incorrect_lut_dists_block32_4b_v2(const uint8_t *codes, const uint8_t *luts,
  ...
  for (int64_t i = 0; i < nblocks; i++) {
    ...
    for (uint8_t j = 0; j < NBytes; j += 2) {
      ...
      // luts += 32;
      // auto lut0 = _mm256_permute2x128_si256(both_luts, both_luts, 0 + (0 <<
      // 4)); auto lut1 = _mm256_permute2x128_si256(both_luts, both_luts, 1 + (1
      // << 4));
    ...
    _mm256_stream_si256((__m256i *)dists_out,
                        totals); // "non-temporal memory hint"
    // luts -= 8 * 64;
    luts -= NBytes / 2 * 32;
    dists_out += 32;
  }
...
template <int NBytes>
inline void
incorrect_lut_dists_block32_4b(const uint8_t *codes, const uint8_t *luts,
                               uint8_t *dists_out, int64_t nblocks) {
  static_assert(NBytes > 0, "Code length <= 0 is not valid");
  static const __m256i low_4bits_mask = _mm256_set1_epi8(0x0F);
  ...
  for (int64_t i = 0; i < nblocks; i++) {
    ...
    for (uint8_t j = 0; j < NBytes; j++) {
      ...
      if (j % 2 == 0) {
        ...
        luts += 32;
        both_luts =
            _mm256_permute2x128_si256(four_luts, four_luts, 0 + (0 << 4));
      } else {
        both_luts =
            _mm256_permute2x128_si256(four_luts, four_luts, 1 + (1 << 4));
      }
      ...
      // auto both_luts = load_si256i(luts);
      // auto lut_low = _mm256_permute2x128_si256(both_luts, both_luts, 0 + (0
      // << 4)); auto lut_high = _mm256_permute2x128_si256(both_luts, both_luts,
      // 1 + (1 << 4));
...
inline void lut_dists_block32_4b_vertical(const uint8_t *codes,
  ...
  static constexpr int nstripes =
      B / packet_width; // # of rows of 32B per block
...
// non-unpacking) func
template <int NBytes>
inline void debug_lut_dists_block32_4b(const uint8_t *codes,
                                       const uint8_t *luts, uint8_t *dists_out,
                                       int64_t nblocks) {
  static_assert(NBytes > 0, "Code length <= 0 is not valid");
...
template <int B, int NBytes,
          class dist_t> // block size, number of dimensions of each vector
...
//             } else if (nbytes_multiple_of_8 && idx_bits_start >= 64 &&
//             idx_bits_end <= 128) {
...
//             } else if (nbytes_multiple_of_8 && idx_bits_start >= 128 &&
//             idx_bits_end <= 192) {
...
//             } else if (nbytes_multiple_of_8 && idx_bits_start >= 192 &&
//             idx_bits_end <= 256) {
...
//             } else {
//                 // // do evil to figure out the uint64 containing the index,
//                 and
...
template <int NBytes, class dist_t>
inline void lut_dists_8b_stride4b(const uint8_t *codes, const dist_t *luts,
                                  dist_t *dists_out, int64_t N) {
  static_assert(NBytes > 0, "Code length <= 0 is not valid");
