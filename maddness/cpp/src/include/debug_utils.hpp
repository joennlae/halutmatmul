...
#define PRINT_DEBUG(STR) std::cout << __func__ << "(): " STR << "\n";
...
#define PRINTLN_VAR(VAR) std::cout << #VAR ":\n" << VAR << "\n";
...
template <typename P> inline int32_t pretty_ptr(P ptr) {
...
// M "\n", __FILE__, __LINE__, clean_errno(), ##__VA_ARGS__) 	#define
// assertf(A, M, ...) if(!(A)) {log_error(M, ##__VA_ARGS__); assert(A); }
// #else
// 	#define assertf(A, M, ...)
// #endif
...
static inline void print(const char *str) { printf("%s\n", str); }
...
// template<class T, class _=typename std::enable_if< !std::is_pointer<T>::value
// >::type>
...
inline void dump_elements(const T *x, size_t len = 1, size_t newline_every = 1,
  ...
  for (size_t i = 0; i < len_elements; i++) {
    ...
    // } else if (((i+1) % 8 == 0) && i + 1 < len_elements) { // write 8B
    // separator unless at very end
    if (write_newline) {
      printf(",\n");
    } else {
  ...
  if (newline_every <= len && ((len_elements % newline_every) != 0)) {
    printf("\n");
  }
}
