...
template <class InputT> struct mithral_amm {
  ...
  void encode(const InputT *X) {
    ...
    mithral_encode(X, N, D, splitdims, splitvals, encode_scales, encode_offsets,
                   ncodebooks, tmp_codes.data());
  ...
  void lut(const float *Q) {
    ...
    if (nnz_per_centroid > 0) {
      mithral_lut_sparse(Q, M, D, ncodebooks, centroids, idxs, nnz_per_centroid,
                         out_offset_sum, out_scale, tmp_luts_f32.data(),
                         luts.data());
    } else {
      // printf("dense lut! ");
      mithral_lut_dense(Q, M, D, ncodebooks, centroids, out_offset_sum,
                        out_scale, tmp_luts_f32.data(), luts.data());
    }
  }
