...
//     // 3 + 3 cycles (_MM_SHUFFLE is a built-in macro that generates
//     constants) tmp = _mm256_max_ps(tmp,
...
//     auto tmp2 = _mm256_min_epi32(tmp,
//     _mm256_shuffle_epi32(tmp,tmp,_MM_SHUFFLE(1,0,3,2))); return
//     pfirst(_mm256_min_epi32(tmp2, _mm256_shuffle_epi32(tmp2,tmp2,1)));
...
static inline void sgemm_colmajor_narrow_padded(
    ...
    bool add_to_output = false, int A_col_stride = -1, int B_col_stride = -1,
    int out_col_stride = -1, int nrows_per_chunk = 512) {
    ...
    for (int m = 0; m < nstripes_M; m++) { // for each group of output cols
      ...
      for (int n = 0; n < nstripes_N; n++) {
        ...
        for (int j = 0; j < D; j++) {
          ...
          for (int nn = 0; nn < InRowTileSz; nn++) {
            // if ((chunk == 0 || chunk == 1 || chunk == (nchunks_N - 1)) && (n
            // == 510 || n == 0)) {
            //     // printf("D = %d\n", D);
            //     printf("row idx: = %d\n", (n * InRowTileSz) + nn);
            //     printf("got to chunk %d / %d\n", chunk, nchunks_N - 1);
            ...
            //     ptr_diff); printf("n, nn = %d, %d; a_row_ptrs[nn] - A_orig:
            //     %ld\n", n, nn, ptr_diff_orig); printf("n, nn = %d, %d; A -
            //     A_orig: %ld\n", n, nn, (long)(A - A_orig));
            // }
        ...
        for (int mm = 0; mm < OutColTileSz; mm++) {
          ...
          // (int)(out_col_ptrs[mm] - out)); printf("n = %d; out_col_ptrs[mm] -
        ...
        //         1, n, nstripes_N - 1); for (int mm = 0; mm < OutColTileSz;
        //         mm++) {
        //             for (int nn = 0; nn < InRowTileSz; nn++) {
        //                 // printf("foo\n");
        //                 printf(" ");
