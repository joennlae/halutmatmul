...
void bolt_lut(const float *q, int len, const float *centroids, uint8_t *out) {
  static_assert(Reduction == Reductions::DistL2 ||
...
void bolt_lut(const float *q, int len, const float *centroids, int ncodebooks,
  ...
  switch (ncodebooks) {
  case 2:
    bolt_lut<1, Reduction>(q, len, centroids, out);
    break;
  case 4:
    bolt_lut<2, Reduction>(q, len, centroids, out);
    break;
  case 8:
    bolt_lut<4, Reduction>(q, len, centroids, out);
    break;
  case 16:
    bolt_lut<8, Reduction>(q, len, centroids, out);
    break;
  case 32:
    bolt_lut<16, Reduction>(q, len, centroids, out);
    break;
  case 64:
    bolt_lut<32, Reduction>(q, len, centroids, out);
    break;
  case 128:
    bolt_lut<32, Reduction>(q, len, centroids, out);
    break;
  default:
    assert(false); // invalid ncodebooks
  }
}
template <int Reduction = Reductions::DistL2>
...
void bolt_lut(const float *q, int len, const float *centroids, int ncodebooks,
  ...
  switch (ncodebooks) {
  case 2:
    bolt_lut<1>(q, len, centroids, offsets, scaleby, out);
    break;
  case 4:
    bolt_lut<2>(q, len, centroids, offsets, scaleby, out);
    break;
  case 8:
    bolt_lut<4>(q, len, centroids, offsets, scaleby, out);
    break;
  case 16:
    bolt_lut<8>(q, len, centroids, offsets, scaleby, out);
    break;
  case 32:
    bolt_lut<16>(q, len, centroids, offsets, scaleby, out);
    break;
  case 64:
    bolt_lut<32>(q, len, centroids, offsets, scaleby, out);
    break;
  case 128:
    bolt_lut<64>(q, len, centroids, offsets, scaleby, out);
    break;
  default:
...
template <int Reduction = Reductions::DistL2>
void bolt_lut(const float *Q, int nrows, int ncols, const float *centroids,
              int ncodebooks, const float *offsets, float scaleby,
              uint8_t *out) {
  auto in_ptr = Q;
  uint8_t *lut_out_ptr = (uint8_t *)out;
  for (int i = 0; i < nrows; i++) {
    bolt_lut(in_ptr, ncols, centroids, ncodebooks, offsets, scaleby,
             lut_out_ptr);
    in_ptr += ncols;
    lut_out_ptr += 16 * ncodebooks;
  }
 ...
 *  of uint8_t
 */
template <int NBytes, bool _ = false,
          bool SignedLUTs = false> // 2nd arg is so bolt<M, false> always works
inline void bolt_scan(const uint8_t *codes, const uint8_t *luts,
...
#pragma unroll
    for (uint8_t j = 0; j < NBytes; j++) {
      ...
      if (NoOverflow) { // convert to epu16s before doing any adds
        if (SignedLUTs) {
          ...
          auto dists16_low_evens =
              _mm256_srai_epi16(_mm256_srai_epi16(dists_low, 8), 8);
          auto dists16_high_evens =
              _mm256_srai_epi16(_mm256_srai_epi16(dists_high, 8), 8);
          ...
          auto dists16_high_evens =
              _mm256_and_si256(dists_high, low_8bits_mask);
        ...
        if (SignedLUTs) {
          ...
          auto dists16_evens =
              _mm256_srai_epi16(_mm256_srai_epi16(dists, 8), 8);
    ...
    auto tmp_low =
        _mm256_permute4x64_epi64(totals_evens, _MM_SHUFFLE(3, 1, 2, 0));
    auto tmp_high =
        _mm256_permute4x64_epi64(totals_odds, _MM_SHUFFLE(3, 1, 2, 0));
...
void bolt_scan(const uint8_t *codes, int64_t nblocks, int ncodebooks,
  ...
  switch (ncodebooks) {
  case 2:
    bolt_scan<1, NoOverflow, SignedLUTs>(codes, luts, dists_out, nblocks);
    break;
  case 4:
    bolt_scan<2, NoOverflow, SignedLUTs>(codes, luts, dists_out, nblocks);
    break;
  case 8:
    bolt_scan<4, NoOverflow, SignedLUTs>(codes, luts, dists_out, nblocks);
    break;
  case 16:
    bolt_scan<8, NoOverflow, SignedLUTs>(codes, luts, dists_out, nblocks);
    break;
  case 32:
    bolt_scan<16, NoOverflow, SignedLUTs>(codes, luts, dists_out, nblocks);
    break;
  case 64:
    bolt_scan<32, NoOverflow, SignedLUTs>(codes, luts, dists_out, nblocks);
    break;
  case 128:
    bolt_scan<64, NoOverflow, SignedLUTs>(codes, luts, dists_out, nblocks);
    break;
  default:
    assert(false); // unsupported ncodebooks
  }
...
template <bool NoOverflow = true, bool SignedLUTs = false, bool tile = true,
          class dist_t>
void bolt_scan(const uint8_t *codes, int64_t nblocks, int ncodebooks,
  ...
  for (int chunk = 0; chunk < nchunks;
    ...
    for (int i = 0; i < noutputs; i++) {
      bolt_scan<NoOverflow, SignedLUTs>(codes_ptr, use_nblocks, ncodebooks,
                                        lut_ptr, out_ptr);
