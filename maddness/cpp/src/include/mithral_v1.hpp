...
void dense_lut_f32_fused(const float *Q, int nrows, int ncols, int ncodebooks,
                         ...
                         float *__restrict__ out_offsets, float &out_offset_sum,
...
void sparse_lut_f32(const float *Q, int nrows, int ncols, int ncodebooks,
                    const float *centroids, const int *idxs,
                    int nnz_per_centroid, float *out);
...
void mithral_lut_sparse(const float *Q, int nrows, int ncols, int ncodebooks,
                        const float *centroids, const int *idxs,
                        int nnz_per_centroid, float &out_offset_sum,
                        float &out_scale, float *__restrict__ tmp_lut_f32,
                        uint8_t *out);
...
void _compute_offsets_scale_from_mins_maxs(const __m256 *mins,
                                           const __m256 *maxs, int ncodebooks,
                                           float *out_offsets,
  ...
  for (int r = 0; r < nrow_blocks; r++) {
    for (int c = 0; c < ncodebook_blocks; c++) {
      for (int cc = 0; cc < CodebookTileSz; cc++) {
        ...
        centroids_ptrs[cc] = centroids + (centroids_codebook_stride * codebook);
        for (int rr = 0; rr < RowTileSz; rr++) {
          // set output ptrs for this codebook
          auto row = (r * RowTileSz) + rr;
          out_ptrs[rr][cc] =
              out + (out_row_stride * row) + (out_codebook_stride * codebook);
        ...
        for (int jj = 0; jj < ColTileSz; jj++) {
          ...
          for (int cc = 0; cc < CodebookTileSz; cc++) {
            for (int s = 0; s < nstripes; s++) {
              ...
              for (int rr = 0; rr < RowTileSz; rr++) {
                accumulators[cc][rr][s] = fma(vbroadcasted[rr], centroids_col,
                                              accumulators[cc][rr][s]);
                // auto idx = cc * (RowTileSz + nstripes) + (rr * RowTileSz) +
                // s; accumulators[idx] = fma(vbroadcasted[rr],
      ...
      for (int jj = ncol_blocks_full * ColTileSz; jj < ncols; jj++) {
        ...
        for (int cc = 0; cc < CodebookTileSz; cc++) {
          for (int s = 0; s < nstripes; s++) {
            ...
            for (int rr = 0; rr < RowTileSz; rr++) {
              accumulators[cc][rr][s] =
                  fma(vbroadcasted[rr], centroids_col, accumulators[cc][rr][s]);
            }
          }
        }
      }
...
// is rowmajor
template <int CodebookTileSz = 2, int RowTileSz = 2>
void _dense_lut_f32_fused(
    const float *Q, int nrows, int ncols, int ncodebooks,
    // const float* centroids, float* out)
      ...
      for (int rr = 0; rr < RowTileSz; rr++) {
        for (int cc = 0; cc < CodebookTileSz; cc++) {
          ...
          for (int s = 0; s < nstripes; s++) {
            ...
            mins[codebook] = _mm256_min_ps(mins[codebook], half_lut);
            maxs[codebook] = _mm256_max_ps(maxs[codebook], half_lut);
...
void dense_lut_f32_fused(
  ...
  if (nrows_round > 0) {
    _dense_lut_f32_fused<CodebookTileSz, RowTileSz>(
        Q, nrows_round, ncols, ncodebooks, mins, maxs, centroids, out_offsets,
  ...
  _dense_lut_f32_fused<CodebookTileSz, 1>(Q, nrows_trailing, ncols, ncodebooks,
...
void mithral_learn_lut_offsets_scales(const float *luts_f32, int nrows,
                                      int ncodebooks, float *out_offsets,
  ...
  for (int row = nrows_round; row < nrows; row++) { // handle trailing rows
    ...
    for (int g = 0; g < ncodebook_groups; g++) {
      for (int gg = 0; gg < codebook_group_sz; gg++) {
        ...
        luts_epi16[0][gg] = _mm256_packs_epi32(vlut_epi32_0, vlut_epi32_1);
      }
...
void quantize_luts(const float *luts_f32, int nrows, int ncodebooks,
  ...
  switch (ncodebooks) {
  case 2:
    quantize_luts<2, RowTileSz>(luts_f32, nrows, offsets, scaleby, out_luts);
    break;
  case 4:
    quantize_luts<4, RowTileSz>(luts_f32, nrows, offsets, scaleby, out_luts);
    break;
  case 8:
    quantize_luts<8, RowTileSz>(luts_f32, nrows, offsets, scaleby, out_luts);
    break;
  case 16:
    quantize_luts<16, RowTileSz>(luts_f32, nrows, offsets, scaleby, out_luts);
    break;
  case 32:
    quantize_luts<32, RowTileSz>(luts_f32, nrows, offsets, scaleby, out_luts);
    break;
  case 64:
    quantize_luts<64, RowTileSz>(luts_f32, nrows, offsets, scaleby, out_luts);
    break;
  case 128:
    quantize_luts<128, RowTileSz>(luts_f32, nrows, offsets, scaleby, out_luts);
    break;
  }
}
...
void mithral_lut_v1(const float *q, int len, int ncodebooks,
  ...
  for (int m = 0; m < ncodebooks; m++) { // for each codebook
    ...
    for (int j = 0; j < round_len; j += j_tile_sz) {
      // for (int jj = j; jj < MIN(len, j + j_tile_sz); jj++) { // for each dim
      // in subvect
    ...
    if (m % 2) {
      ...
      auto dists = packed_epu16_to_unpacked_epu8(dists_uint16_0, dists_uint16);
...
inline void mithral_scan_unpacked_colmajor(const uint8_t *codes,
                                           int64_t nblocks, int ncodebooks,
                                           const uint8_t *luts, int16_t *out) {
  static constexpr int lut_sz = 16;
  ...
  for (int m = 0; m < ncodebooks; m++) {
    ...
    auto vlut =
        _mm256_broadcastsi128_si256(load_si128i((const __m128i *)lut_ptr));
    ...
    for (int64_t b = 0; b < nblocks; b++) {
      ...
      auto dists0 = _mm256_cvtepi8_epi16(_mm256_extracti128_si256(dists, 0));
      auto dists1 = _mm256_cvtepi8_epi16(_mm256_extracti128_si256(dists, 1));
...
void _accumulate_8bit_si256(const __m256i &x0, const __m256i &x1,
                            ...
                            const __m256i &initial_16_31, __m256i &out_0_15,
                            __m256i &out_16_31) {
  ...
  if (SignedLUTs) {
    if (UpcastEvery == 4) {
      ...
      out_0_15 = _mm256_cvtepi8_epi16(_mm256_extracti128_si256(x, 0));
      out_16_31 = _mm256_cvtepi8_epi16(_mm256_extracti128_si256(x, 1));
    } else if (UpcastEvery == 2) {
      // pairwise sums, then upcast to 16 bits
      auto x01 = _mm256_adds_epi8(x0, x1);
      auto x23 = _mm256_adds_epi8(x2, x3);

      auto x01_0 = _mm256_cvtepi8_epi16(_mm256_extracti128_si256(x01, 0));
      auto x01_1 = _mm256_cvtepi8_epi16(_mm256_extracti128_si256(x01, 1));
      auto x23_0 = _mm256_cvtepi8_epi16(_mm256_extracti128_si256(x23, 0));
      auto x23_1 = _mm256_cvtepi8_epi16(_mm256_extracti128_si256(x23, 1));
      ...
      auto x0_0 = _mm256_cvtepi8_epi16(_mm256_extracti128_si256(x0, 0));
      auto x0_1 = _mm256_cvtepi8_epi16(_mm256_extracti128_si256(x0, 1));
      auto x1_0 = _mm256_cvtepi8_epi16(_mm256_extracti128_si256(x1, 0));
      auto x1_1 = _mm256_cvtepi8_epi16(_mm256_extracti128_si256(x1, 1));
      auto x2_0 = _mm256_cvtepi8_epi16(_mm256_extracti128_si256(x2, 0));
      auto x2_1 = _mm256_cvtepi8_epi16(_mm256_extracti128_si256(x2, 1));
      auto x3_0 = _mm256_cvtepi8_epi16(_mm256_extracti128_si256(x3, 0));
      auto x3_1 = _mm256_cvtepi8_epi16(_mm256_extracti128_si256(x3, 1));
    ...
    if (UpcastEvery == 4) {
      ...
      out_0_15 = _mm256_cvtepu8_epi16(_mm256_extracti128_si256(x, 0));
      out_16_31 = _mm256_cvtepu8_epi16(_mm256_extracti128_si256(x, 1));
    } else if (UpcastEvery == 2) {
      // pairwise sums, then upcast to 16 bits
      auto x01 = _mm256_adds_epu8(x0, x1);
      auto x23 = _mm256_adds_epu8(x2, x3);

      auto x01_0 = _mm256_cvtepu8_epi16(_mm256_extracti128_si256(x01, 0));
      auto x01_1 = _mm256_cvtepu8_epi16(_mm256_extracti128_si256(x01, 1));
      auto x23_0 = _mm256_cvtepu8_epi16(_mm256_extracti128_si256(x23, 0));
      auto x23_1 = _mm256_cvtepu8_epi16(_mm256_extracti128_si256(x23, 1));

      out_0_15 = _mm256_adds_epi16(x01_0, x23_0);
      out_16_31 = _mm256_adds_epi16(x01_1, x23_1);
    } else if (UpcastEvery == 1) {
      // convert everything to 16 bits immediately
      auto x0_0 = _mm256_cvtepu8_epi16(_mm256_extracti128_si256(x0, 0));
      auto x0_1 = _mm256_cvtepu8_epi16(_mm256_extracti128_si256(x0, 1));
      auto x1_0 = _mm256_cvtepu8_epi16(_mm256_extracti128_si256(x1, 0));
      auto x1_1 = _mm256_cvtepu8_epi16(_mm256_extracti128_si256(x1, 1));
      auto x2_0 = _mm256_cvtepu8_epi16(_mm256_extracti128_si256(x2, 0));
      auto x2_1 = _mm256_cvtepu8_epi16(_mm256_extracti128_si256(x2, 1));
      auto x3_0 = _mm256_cvtepu8_epi16(_mm256_extracti128_si256(x3, 0));
      auto x3_1 = _mm256_cvtepu8_epi16(_mm256_extracti128_si256(x3, 1));
...
inline void _mithral_scan_tile4(const uint8_t *codes, int64_t nblocks,
                                int ncodebooks, const LutT *luts,
                                int16_t *out) {
  ...
  for (int t = 0; t < ntiles; t++) {
    ...
    for (int64_t b = 0; b < nblocks; b++) {
      ...
      if (Packed) {
        ...
        vcodes1 =
            _mm256_and_si256(_mm256_srli_epi16(vcodes01, 4), low_4bits_mask);
        vcodes2 = _mm256_and_si256(vcodes23, low_4bits_mask);
        vcodes3 =
            _mm256_and_si256(_mm256_srli_epi16(vcodes23, 4), low_4bits_mask);
      } else {
      ...
      _accumulate_8bit_si256<UpcastEvery, SignedLUTs>(
          dists0, dists1, dists2, dists3, dists_so_far_0_15, dists_so_far_16_31,
...
inline void mithral_scan_tile4(const uint8_t *codes, int64_t nblocks,
                               int ncodebooks, const LutT *luts, int16_t *out) {
  _mithral_scan_tile4<UpcastEvery, true>(codes, nblocks, ncodebooks, luts, out);
}
...
template <int NReadCols, int NWriteCols = 1, int UpcastEvery = 4,
          int NCodebooks = -1, int ChunkSz = 256>
inline void
_mithral_scan_tiled(const uint8_t *codes, int nrows, int ncodebooks,
                    int noutputs, const int8_t *luts, int16_t *out,
                    bool add_to_output = false, int codes_col_stride = -1,
                    int lut_col_stride = -1, int out_col_stride = -1,
...
// int nrows_per_chunk=128)
{
  ...
  lut_col_stride =
      lut_col_stride >= 1 ? lut_col_stride : default_lut_col_stride;
  ...
  for (int n = 0; n < nchunks_N; n++) {
    ...
    for (int m = 0; m < nstripes_out; m++) { // for each group of outputs
      ...
      for (int mm = 0; mm < NWriteCols; mm++) {
        ...
        if (!add_to_output &&
            NCodebooks != 4) { // zero this block of output buffer
          for (int i = 0; i < N; i++) {
      ...
      for (int j = 0; j < ncolstripes_in; j++) {
        ...
        for (int mm = 0; mm < NWriteCols; mm++) {
          ...
          for (int jj = 0; jj < NReadCols; jj++) {
            ...
            // if (m == 0) {
            //     printf("LUT for outcol=%2d, incol=%2d, mm=%2d, jj=%2d,
            //     lut_ab, lut_cd:\n", m * NWriteCols + mm, j * NReadCols + jj,
            //     mm, jj); dump_m256i<int8_t>(vluts[jj][mm][0]);
        ...
        for (int b = 0; b < nblocks_N; b++) { // for each block of rows
          ...
          for (int jj = 0; jj < NReadCols; jj++) { // for each in col
            ...
            auto vcodes_ab = _mm256_and_si256(vcodes_packed, low_4bits_mask);
            ...
            for (int mm = 0; mm < NWriteCols; mm++) { // each out col
              ...
              if (UpcastEvery == 2) { // just immediately upcast
                if (evens_odds) {
                  ...
                  auto sum_ac_bd_evens =
                      _mm256_srai_epi16(_mm256_slli_epi16(sum_ac_bd, 8), 8);
                  auto sum_ac_bd_odds = _mm256_srai_epi16(sum_ac_bd, 8);
                  sums_evens[mm] =
                      _mm256_adds_epi16(sums_evens[mm], sum_ac_bd_evens);
                  sums_odds[mm] =
                      _mm256_adds_epi16(sums_odds[mm], sum_ac_bd_odds);
                } else {
                ...
                sums_8b[mm] = _mm256_adds_epi8(sums_8b[mm], sum_ac_bd);
              }
            }
            auto needs_upcast =
                (jj == NReadCols - 1) ||            // last col
          ...
          for (int mm = 0; mm < NWriteCols; mm++) {
            ...
            if (evens_odds) {
              ...
              auto tmp_odds = _mm256_permute4x64_epi64(sums_odds[mm],
                                                       _MM_SHUFFLE(3, 1, 2, 0));
              auto low_sums = _mm256_unpacklo_epi16(tmp_evens, tmp_odds);
              auto high_sums = _mm256_unpackhi_epi16(tmp_evens, tmp_odds);
              sums[mm] = _mm256_adds_epi16(sums[mm], low_sums);
              sums[mm] = _mm256_adds_epi16(sums[mm], high_sums);
            }
  ...
  if (ncols == 1) {
    ...
    if (noutputs % 5 == 0 && UpcastEvery > 2) {
      _mithral_scan_tiled<1, 5, UpcastEvery, NCodebooks>(
          codes, nrows, ncodebooks, noutputs, luts, out);
      return;
    ...
    _mithral_scan_tiled<2, 2, UpcastEvery, NCodebooks>(codes, nrows, ncodebooks,
                                                       noutputs, luts, out);
    return;
    // } else if (ncols % 6 == 0) {
    ...
    _mithral_scan_tiled<5, 1, UpcastEvery>(codes, nrows, ncodebooks, noutputs,
                                           luts, out);
    return;
  } else if (ncols % 4 == 0 && UpcastEvery > 2) {
    _mithral_scan_tiled<4, 1, UpcastEvery>(codes, nrows, ncodebooks, noutputs,
                                           luts, out);
    return;
  } else if (ncols % 3 == 0 && UpcastEvery > 2) {
    _mithral_scan_tiled<3, 1, UpcastEvery>(codes, nrows, ncodebooks, noutputs,
                                           luts, out);
    return;
  } else if (ncols % 2 == 0) {
    _mithral_scan_tiled<2, 1, UpcastEvery>(codes, nrows, ncodebooks, noutputs,
                                           luts, out);
    return;
  } else {
    _mithral_scan_tiled<1, 1, UpcastEvery>(codes, nrows, ncodebooks, noutputs,
                                           luts, out);
    return;
  }
}
...
void mithral_scan(const uint8_t *codes, int64_t nblocks, const uint8_t *luts,
  ...
  // static_assert(actually_upcast_every == 2 || UpcastEvery == 4 || UpcastEvery
  // == 8, "UpcastEvery must be <= 16");
...
void mithral_scan(const uint8_t *codes, int64_t nblocks, int ncodebooks,
  ...
  switch (ncodebooks) {
  case 4:
    mithral_scan<2, UpcastEvery>(codes, nblocks, luts, out);
    break;
  case 8:
    mithral_scan<4, UpcastEvery>(codes, nblocks, luts, out);
    break;
  case 16:
    mithral_scan<8, UpcastEvery>(codes, nblocks, luts, out);
    break;
  case 32:
    mithral_scan<16, UpcastEvery>(codes, nblocks, luts, out);
    break;
  case 64:
    mithral_scan<32, UpcastEvery>(codes, nblocks, luts, out);
    break;
  case 128:
    mithral_scan<64, UpcastEvery>(codes, nblocks, luts, out);
    break;
  }
}
