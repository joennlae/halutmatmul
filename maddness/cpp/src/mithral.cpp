...
void zip_bolt_colmajor(const uint8_t *codes_in, int64_t nrows,
  ...
  // }
  if (ncodebooks % 8 == 0) {
    zip_bolt_colmajor<8>(codes_in, nrows, ncodebooks, codes_out);
    return;
  }
  if (ncodebooks % 4 == 0) {
    zip_bolt_colmajor<4>(codes_in, nrows, ncodebooks, codes_out);
    return;
  }
...
void dense_lut_f32_fused(
  ...
  // assert(nrows % row_tile_sz == 0);  // TODO handle trailing rows in fused
  // func
  ...
  dense_lut_f32_fused<codebook_tile_sz, row_tile_sz>(
      Q, nrows, ncols, ncodebooks, centroids, out_offsets, out_offset_sum,
      out_scale, out);
}
...
void dense_lut_f32(const float *Q, int nrows, int ncols, int ncodebooks,
  ...
  if (nrows_round > 0) {
    dense_lut_f32<CodebookTileSz, RowTileSz>(Q, nrows_round, ncols, ncodebooks,
                                             centroids, out);
  }
  ...
  if (nrows_trailing > 0) {
    dense_lut_f32<CodebookTileSz, 1>(Q, nrows_trailing, ncols, ncodebooks,
                                     centroids, out);
  }
...
void sparse_lut_f32(const float *Q, int nrows, int ncols, int ncodebooks,
  ...
  auto nrows_round = nrows - nrows_trailing;
  if (nrows_round > 0) {
    sparse_lut_f32<CodebookTileSz, RowTileSz>(Q, nrows_round, ncols, ncodebooks,
                                              centroids, idxs, nnz_per_centroid,
                                              out);
  }
  // handle trailing rows
  auto q_row_stride = ncols;
  ...
  // rm handling of the trailing rows entirely, code is twice as fast
  if (nrows_trailing > 0) {
    sparse_lut_f32<CodebookTileSz, 1>(Q, nrows_trailing, ncols, ncodebooks,
...
void mithral_lut_dense(const float *Q, int nrows, int ncols, int ncodebooks,
  ...
  dense_lut_f32_fused(Q, nrows, ncols, ncodebooks, centroids, tmp_offsets,
                      out_offset_sum, out_scale, tmp_lut_f32);
...
void mithral_lut_sparse(const float *Q, int nrows, int ncols, int ncodebooks,
  ...
  // unfused stats computation
  //
  sparse_lut_f32(Q, nrows, ncols, ncodebooks, centroids, idxs, nnz_per_centroid,
                 tmp_lut_f32);
  float tmp_offsets[ncodebooks];
  mithral_learn_lut_offsets_scales(tmp_lut_f32, nrows, ncodebooks, tmp_offsets,
...
void mithral_scan(const uint8_t *codes, int64_t nblocks, int ncodebooks,
  ...
  // mithral_scan<128, 2>(codes, nblocks, ncodebooks, noutputs, luts,
  // dists_out);
  ...
  //     dists_out);
  // } else {
  //     mithral_scan<128, 1>(codes, nblocks, ncodebooks, noutputs, luts,
  //     dists_out);
  // }
}

// void mithral_scan_notile(const uint8_t* codes, int64_t nblocks, int
// ncodebooks,
// // void mithral_scan(const uint8_t* codes, int64_t nblocks, int ncodebooks,
//                   int noutputs, const uint8_t* luts, uint8_t* dists_out)
// {
...
// ncodebooks, void mithral_scan(const uint8_t* codes, int64_t nblocks, int
// ncodebooks,
//                   int noutputs, const uint8_t* luts, uint8_t* dists_out)
// {
...
//     // static constexpr int target_chunk_nbytes = 1 << 14;  // 16kiB for L1
//     cache
//     // static constexpr int target_chunk_nbytes = 24 * 1024;  // 24kiB for L1
//     cache
//     // static constexpr int target_chunk_nbytes = 31 * 1024;  // 31kiB for L1
//     cache
...
//     for (int chunk = 0; chunk < nchunks; chunk++) { // for each chunk of
//     input rows
...
//         for (int i = 0; i < noutputs; i++) {
//             mithral_scan(codes_ptr, use_nblocks, ncodebooks, lut_ptr,
//             out_ptr); out_ptr += out_col_stride; lut_ptr += lut_col_stride;
//         }
//     }
// }
