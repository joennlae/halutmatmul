...
#define PROFILE_DIST_COMPUTATION(NAME, NTRIALS, DISTS_PTR, NUM_DISTS, EXPR)    \
  do {                                                                         \
    ...
    print_dist_stats(NAME " (best of " #NTRIALS ")", NUM_DISTS, __t_min);      \
  } while (0);

#define REPEATED_PROFILE_DIST_COMPUTATION(NREPS, NAME, NTRIALS, DISTS_PTR,     \
                                          NUM_DISTS, EXPR)                     \
  do {                                                                         \
...
#define PROFILE_DIST_COMPUTATION_LOOP(NAME, NTRIALS, DISTS_PTR, NUM_DISTS,     \
                                      NUM_LOOP_ITERS, EXPR)                    \
  do {                                                                         \
    double __t_min = std::numeric_limits<double>::max();                       \
    for (int __i = 0; __i < NTRIALS; __i++) {                                  \
    ...
    print_dist_stats(NAME " (best of " #NTRIALS ")", NUM_LOOP_ITERS,           \
                     __t_min / 1000.0);                                        \
  } while (0);
