...
#pragma mark 4bit codes
  ...
  REPEATED_PROFILE_DIST_COMPUTATION(
      kNreps, "built-in popcnt", kNtrials, dists_popcnt, N,
  ...
  PROFILE_DIST_COMPUTATION(
      "naive 4b lut", 5, dists_scalar, N,
      dist::debug_lut_dists_block32_4b<M>(block_codes, popcount_luts32,
                                          dists_scalar, nblocks));

  PROFILE_DIST_COMPUTATION(
      "vectorized 4b lut", 5, dists_vector, N,
      dist::lut_dists_block32_4b<M>(block_codes, popcount_luts32, dists_vector,
                                    nblocks));

  PROFILE_DIST_COMPUTATION(
      "vect unpack 4b lut", 5, dists_unpack, N,
      dist::lut_dists_block32_4b_unpack<M>(block_codes, popcount_luts16,
                                           dists_unpack, nblocks));

#pragma mark four_bit vertical
  // note that we aren't passing in correct codes to these, except the first

  PROFILE_DIST_COMPUTATION(
      "32 vertical", 5, dists_vertical32, N,
      (dist::lut_dists_block32_4b_vertical<32, M>(block_codes, popcount_luts16,
                                                  dists_vertical32, N)));

  PROFILE_DIST_COMPUTATION(
      "64 vertical", 5, dists_vertical64, N,
      (dist::lut_dists_block32_4b_vertical<64, M>(block_codes, popcount_luts16,
                                                  dists_vertical64, N)));

  PROFILE_DIST_COMPUTATION(
      "128 vertical", 5, dists_vertical128, N,
      (dist::lut_dists_block32_4b_vertical<128, M>(block_codes, popcount_luts16,
                                                   dists_vertical128, N)));

  PROFILE_DIST_COMPUTATION(
      "256 vertical", 5, dists_vertical256, N,
      (dist::lut_dists_block32_4b_vertical<256, M>(block_codes, popcount_luts16,
                                                   dists_vertical256, N)));
...
#pragma mark 8bit vertical
  ...
  PROFILE_DIST_COMPUTATION("t 32 8b, 8b dist", 5, dists_8b_b, N,
                           (dist::lut_dists_8b_vertical<32, M>(
                               block_codes, popcount_luts256b, dists_8b_b, N)));
...
#pragma mark 12bit codes
  ...
  PROFILE_DIST_COMPUTATION(
      "t 12b, 8b dist", 5, dists_12b_b, N,
      dist::lut_dists_12b<M>(codes, popcount_luts12b_b, dists_12b_b, N));

  PROFILE_DIST_COMPUTATION(
      "t 12b, 16b dist", 5, dists_12b_s, N,
      dist::lut_dists_12b<M>(codes, popcount_luts12b_s, dists_12b_s, N));

  PROFILE_DIST_COMPUTATION(
      "t 12b, 16b dist", 5, dists_12b_i, N,
      dist::lut_dists_12b<M>(codes, popcount_luts12b_i, dists_12b_i, N));

#pragma mark 12bit vertical

  PROFILE_DIST_COMPUTATION(
      "t 16 12b, 8b dist", 5, dists_12b_b, N,
      (dist::lut_dists_12b_vertical<16, M>(block_codes, popcount_luts12b_b,
                                           dists_12b_b, N)));

  PROFILE_DIST_COMPUTATION(
      "t 64 12b, 8b dist", 5, dists_12b_b, N,
      (dist::lut_dists_12b_vertical<64, M>(block_codes, popcount_luts12b_b,
                                           dists_12b_b, N)));

  PROFILE_DIST_COMPUTATION(
      "t 256 12b, 8b dist", 5, dists_12b_b, N,
      (dist::lut_dists_12b_vertical<256, M>(block_codes, popcount_luts12b_b,
                                            dists_12b_b, N)));
...
#pragma mark 16bit codes
  ...
  int lut_sz_16b = (1 << 16);
  uint8_t *popcount_luts16b_b =
      aligned_random_ints<uint8_t>(M / 2 * lut_sz_16b);
  uint16_t *popcount_luts16b_s =
      aligned_random_ints<uint16_t>(M / 2 * lut_sz_16b);
  uint32_t *popcount_luts16b_i =
      aligned_random_ints<uint32_t>(M / 2 * lut_sz_16b);

  uint16_t *codes16 = (uint16_t *)codes;
  uint16_t *block_codes16 = (uint16_t *)block_codes;

  PROFILE_DIST_COMPUTATION(
      "t 16b, 8b dist", 5, dists_16b_b, N,
      dist::lut_dists_16b<M>(codes16, popcount_luts16b_b, dists_16b_b, N));

  PROFILE_DIST_COMPUTATION(
      "t 16b, 16b dist", 5, dists_16b_s, N,
      dist::lut_dists_16b<M>(codes16, popcount_luts16b_s, dists_16b_s, N));

  PROFILE_DIST_COMPUTATION(
      "t 16b, 32b dist", 5, dists_16b_i, N,
      dist::lut_dists_16b<M>(codes16, popcount_luts16b_i, dists_16b_i, N));

  PROFILE_DIST_COMPUTATION(
      "t 32 16b, 8b dist", 5, dists_16b_b, N,
      (dist::lut_dists_16b_vertical<32, M>(block_codes16, popcount_luts16b_b,
                                           dists_16b_b, N)));

  PROFILE_DIST_COMPUTATION(
      "t 256 16b, 8b dist", 5, dists_16b_b, N,
      (dist::lut_dists_16b_vertical<256, M>(block_codes16, popcount_luts16b_b,
                                            dists_16b_b, N)));

  PROFILE_DIST_COMPUTATION(
      "t 1024 16b, 8b dist", 5, dists_16b_b, N,
      (dist::lut_dists_16b_vertical<1024, M>(block_codes16, popcount_luts16b_b,
                                             dists_16b_b, N)));
