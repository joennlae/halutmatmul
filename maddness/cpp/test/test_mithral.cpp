...
void _zip4_one_block(const MatT0 &codes_in, MatT1 &out, int in_row_offset = 0,
                     int out_row_offset = 0, int in_col_offset = 0,
  ...
  for (int i = 0; i < lut_sz; i++) {
    out(i + out_offset, j_out) = (codes_in(i + in_offset, 2 + j_in) << 4) +
                                 codes_in(i + in_offset, j_in);
  }
  // bd
  out_offset += lut_sz;
  for (int i = 0; i < lut_sz; i++) {
    out(i + out_offset, j_out) = (codes_in(i + in_offset, 3 + j_in) << 4) +
...
void _test_zip4(int nblocks, int ncodebooks) {
  ...
  codes_in.setRandom();
  codes_in =
      codes_in.unaryExpr([=](const uint8_t x) { return (uint8_t)(x % 16); });
  // for (int i = 0; i < N; i++) {
  //     for (int j = 0; j < D; j++) {
...
void _test_mithral_scan_tiled(int nblocks, int ncodebooks, int nout = 1) {
  ...
  mithral_scan_tiled<2>(
      codes_zipped.data(), N, ncodebooks, nout,
      // _mithral_scan<1, 1, UpcastEvery>(codes_zipped.data(), N, ncodebooks,
      // nout, _mithral_scan<1, 2, UpcastEvery>(codes_zipped.data(), N,
      // ncodebooks, nout, _mithral_scan<2, 1, UpcastEvery>(codes_zipped.data(),
      // N, ncodebooks, nout, _mithral_scan<2, 2,
      // UpcastEvery>(codes_zipped.data(), N, ncodebooks, nout,
      luts.data(), out.data());
