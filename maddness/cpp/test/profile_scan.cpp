...
void _profile_scan_popcount(int nrows, int nbytes, int M = 1) {
  ...
  RowMatrix<uint64_t> A(nrows, ncols);
  A.setRandom();
  ColMatrix<uint64_t> B(ncols, M);
  B.setRandom();
  ColMatrix<uint16_t> out(nrows, M);
  out.setZero();

  std::string msg(string_with_format("%-22s, N C B M:, %7d,  -1, %2d, %2d,\t",
                                     "popcount scan", nrows, nbytes, M));

  REPEATED_PROFILE_DIST_COMPUTATION(
      kNreps, msg, kNtrialsScan, out.data(), out.size(),
...
TEST_CASE("popcnt scan timing", "[amm][scan][popcount][profile][old]") {
  ...
  RowMatrix<uint64_t> A(N, D);
  A.setRandom();
  ColMatrix<uint64_t> B(D, M);
  B.setRandom();
  ColMatrix<uint16_t> out(N, M);
  out.setZero();
  ...
  // } while(0) ));
  REPEATED_PROFILE_DIST_COMPUTATION(
      kNreps, "popcount amm", kNtrials, out.data(), out.size(),
      (bgemm(A.data(), B.data(), N, D, M, out.data())));
  REPEATED_PROFILE_DIST_COMPUTATION(
      kNreps, "popcount amm D=1, 1,1", kNtrials, out.data(), out.size(),
      (_bgemm<1, 1, D>(A.data(), B.data(), N, D, M, out.data())));
  // (_bgemm<1, 1, -1>(A.data(), B.data(), N, D, M, out.data()) ));
  REPEATED_PROFILE_DIST_COMPUTATION(
      kNreps, "popcount amm D=1, 1,2", kNtrials, out.data(), out.size(),
      (_bgemm<1, 2, D>(A.data(), B.data(), N, D, M, out.data())));
  // (_bgemm<1, 2, -1>(A.data(), B.data(), N, D, M, out.data()) ));
  // REPEATED_PROFILE_DIST_COMPUTATION(kNreps, "popcount amm D=1, 1,3",
  // kNtrials,
  //     out.data(), out.size(),
  //     (_bgemm<1, 3, D>(A.data(), B.data(), N, D, M, out.data()) ));
  // REPEATED_PROFILE_DIST_COMPUTATION(kNreps, "popcount amm D=1, 1,4",
  // kNtrials,
  //     out.data(), out.size(),
  //     (_bgemm<1, 4, D>(A.data(), B.data(), N, D, M, out.data()) ));
  REPEATED_PROFILE_DIST_COMPUTATION(
      kNreps, "popcount amm D=1, 2,1", kNtrials, out.data(), out.size(),
      (_bgemm<2, 1, D>(A.data(), B.data(), N, D, M, out.data())));
  // (_bgemm<2, 1, -1>(A.data(), B.data(), N, D, M, out.data()) ));
  REPEATED_PROFILE_DIST_COMPUTATION(
      kNreps, "popcount amm D=1, 2,2", kNtrials, out.data(), out.size(),
      (_bgemm<2, 2, D>(A.data(), B.data(), N, D, M, out.data())));
  // (_bgemm<2, 2, -1>(A.data(), B.data(), N, D, M, out.data()) ));
  // REPEATED_PROFILE_DIST_COMPUTATION(kNreps, "popcount amm D=1, 2,3",
  // kNtrials,
  //     out.data(), out.size(),
  //     (_bgemm<2, 3, D>(A.data(), B.data(), N, D, M, out.data()) ));
  // REPEATED_PROFILE_DIST_COMPUTATION(kNreps, "popcount amm D=1, 2,4",
  // kNtrials,
  //     out.data(), out.size(),
  //     (_bgemm<2, 4, D>(A.data(), B.data(), N, D, M, out.data()) ));
  REPEATED_PROFILE_DIST_COMPUTATION(
      kNreps, "popcount amm D=1, 3,1", kNtrials, out.data(), out.size(),
      (_bgemm<3, 1, D>(A.data(), B.data(), N, D, M, out.data())));
  // (_bgemm<3, 1, -1>(A.data(), B.data(), N, D, M, out.data()) ));
  REPEATED_PROFILE_DIST_COMPUTATION(
      kNreps, "popcount amm D=1, 3,2", kNtrials, out.data(), out.size(),
      (_bgemm<3, 2, D>(A.data(), B.data(), N, D, M, out.data())));
  // (_bgemm<3, 2, -1>(A.data(), B.data(), N, D, M, out.data()) ));
  // REPEATED_PROFILE_DIST_COMPUTATION(kNreps, "popcount amm D=1, 3,3",
  // kNtrials,
  //     out.data(), out.size(),
  //     (_bgemm<3, 3, D>(A.data(), B.data(), N, D, M, out.data()) ));
  // REPEATED_PROFILE_DIST_COMPUTATION(kNreps, "popcount amm D=1, 3,4",
  // kNtrials,
  //     out.data(), out.size(),
  //     (_bgemm<3, 4, D>(A.data(), B.data(), N, D, M, out.data()) ));
  // REPEATED_PROFILE_DIST_COMPUTATION(kNreps, "popcount amm D=1, 4,1",
  // kNtrials,
  //     out.data(), out.size(),
  //     (_bgemm<4, 1, D>(A.data(), B.data(), N, D, M, out.data()) ));
  // // REPEATED_PROFILE_DIST_COMPUTATION(kNreps, "popcount amm D=1, 4,2",
  // kNtrials,
  // //     out.data(), out.size(),
  // //     (_bgemm<4, 2, D>(A.data(), B.data(), N, D, M, out.data()) ));
  // // REPEATED_PROFILE_DIST_COMPUTATION(kNreps, "popcount amm D=1, 4,3",
  // kNtrials,
  // //     out.data(), out.size(),
  // //     (_bgemm<4, 3, D>(A.data(), B.data(), N, D, M, out.data()) ));
  // // REPEATED_PROFILE_DIST_COMPUTATION(kNreps, "popcount amm D=1, 4,4",
  // kNtrials,
  // //     out.data(), out.size(),
  // //     (_bgemm<4, 4, D>(A.data(), B.data(), N, D, M, out.data()) ));
  // REPEATED_PROFILE_DIST_COMPUTATION(kNreps, "popcount amm D=1, 5,1",
  // kNtrials,
  //     out.data(), out.size(),
  //     (_bgemm<5, 1, D>(A.data(), B.data(), N, D, M, out.data()) ));
  // REPEATED_PROFILE_DIST_COMPUTATION(kNreps, "popcount amm D=1, 6,1",
  // kNtrials,
  //     out.data(), out.size(),
...
void _profile_scan_all(int nrows, int nbytes, int nout = 1) {
  ...
  // for mithral / bolt
  ColMatrix<uint8_t> codes16(nrows, ncodebooks);
  codes16.setRandom();
  codes16 =
      codes16.unaryExpr([=](const uint8_t x) { return (uint8_t)(x % 16); });
  ColMatrix<uint8_t> luts16(16, ncodebooks * nout);
  luts16.setRandom();
  ...
  REPEATED_PROFILE_DIST_COMPUTATION(
      kNreps, msg, kNtrialsScan, dists_u8_x2.data(), dists_u8_x2.size(),
      (mithral_scan<2, 1>(codes16.data(), nblocks, ncodebooks, nout,
                          luts16.data(), dists_u8_x2.data())));
...
void _profile_scan(int nrows, int nbytes, int nout = 1) {
  ...
  codes256.setRandom();
  codes256 =
      codes256.unaryExpr([=](const uint8_t x) { return (uint8_t)(x % 256); });
  ColMatrix<float> luts_f32(256, ncodebooks_pq * nout);
  luts_f32.setRandom();
  ColMatrix<float> dists_f32(nrows, nout);
  dists_f32.setRandom();
  ...
  REPEATED_PROFILE_DIST_COMPUTATION(
      kNreps, msg, kNtrialsScan, dists_u8_x2.data(), dists_u8_x2.size() / 2,
      (mithral_scan<128, 2>(codes16.data(), nblocks, ncodebooks, nout,
                            luts16.data(), dists_u8_x2.data())));
}
