...
void _profile_encode(int N, int D, int nbytes) {
  ...
  RowVector<uint32_t> splitdims =
      splitdims_.unaryExpr([=](const uint32_t x) { return x % D; });
  // RowVector<uint32_t> splitdims(total_nsplits); splitdims.setZero(); // TODO
  // rm
  ...
  REPEATED_PROFILE_DIST_COMPUTATION(
      kNreps, msg, kNtrials, out.data(), out.size(),
      mithral_encode(X.data(), N, D, splitdims.data(), all_splitvals.data(),
                     scales.data(), offsets.data(), ncodebooks, out.data()));

  msg = string_with_format(fmt, "mithral encode i8", ncodebooks);
  REPEATED_PROFILE_DIST_COMPUTATION(
      kNreps, msg, kNtrials, out.data(), out.size(),
      mithral_encode(X_i8.data(), N, D, splitdims.data(), all_splitvals.data(),
                     ncodebooks, out.data()));

  msg = string_with_format(fmt, "mithral encode i16", ncodebooks);
  REPEATED_PROFILE_DIST_COMPUTATION(
      kNreps, msg, kNtrials, out.data(), out.size(),
      mithral_encode(X_i16.data(), N, D, splitdims.data(), all_splitvals.data(),
                     shifts.data(), offsets_i16.data(), ncodebooks,
                     out.data()));
}
