...
# type: ignore
# pylint: disable=global-statement, attribute-defined-outside-init, unused-variable, arguments-renamed, dangerous-default-value, unused-argument
import itertools
import torch
from models.levit.utils import replace_batchnorm
...
@register_model
def LeViT_c_128S(num_classes=1000, distillation=True, pretrained=False, fuse=False):
    return model_factory(
        **specification["LeViT_c_128S"],
...
@register_model
def LeViT_c_128(num_classes=1000, distillation=True, pretrained=False, fuse=False):
    return model_factory(
        **specification["LeViT_c_128"],
        num_classes=num_classes,
        distillation=distillation,
        pretrained=pretrained,
        fuse=fuse
...
@register_model
def LeViT_c_192(num_classes=1000, distillation=True, pretrained=False, fuse=False):
    return model_factory(
        **specification["LeViT_c_192"],
        num_classes=num_classes,
        distillation=distillation,
        pretrained=pretrained,
        fuse=fuse
...
@register_model
def LeViT_c_256(num_classes=1000, distillation=True, pretrained=False, fuse=False):
    return model_factory(
        **specification["LeViT_c_256"],
        num_classes=num_classes,
        distillation=distillation,
        pretrained=pretrained,
        fuse=fuse
...
@register_model
def LeViT_c_384(num_classes=1000, distillation=True, pretrained=False, fuse=False):
    return model_factory(
        **specification["LeViT_c_384"],
        num_classes=num_classes,
        distillation=distillation,
        pretrained=pretrained,
        fuse=fuse
...
class Attention(torch.nn.Module):
    def __init__(
        ...
        self.proj = torch.nn.Sequential(
            activation(),
            Conv2d_BN(self.dh, dim, bn_weight_init=0, resolution=resolution),
        )
            ...
            if do[0] == "Subsample":
                ...
                if do[4] > 0:  # mlp_ratio
                    ...
                    self.blocks.append(
                        Residual(
                            torch.nn.Sequential(
                                Conv2d_BN(embed_dim[i + 1], h, resolution=resolution),
                                mlp_activation(),
                                Conv2d_BN(
                                    h,
                                    embed_dim[i + 1],
